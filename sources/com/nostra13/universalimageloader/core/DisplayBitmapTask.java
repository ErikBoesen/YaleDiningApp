    ...
    public void run() {
        if (isViewWasReused()) {
            if (this.loggingEnabled) {
                C0214L.m414i("ImageView is reused for another image. Task is cancelled. [%s]", this.memoryCacheKey);
            }
            this.listener.onLoadingCancelled(this.imageUri, this.imageView);
            return;
        }
        if (this.loggingEnabled) {
            C0214L.m414i("Display image in ImageView [%s]", this.memoryCacheKey);
        }
